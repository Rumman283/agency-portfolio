...
import { deletePortfolioProject } from '@/app/admin/portfolio/actions';
...
export default function DeleteProjectButton({ projectId }: { projectId: string }) {
  ...
  async function handleDelete() {
    ...
    try {
      const result = await deletePortfolioProject(projectId);
      if (result?.error) {
        ...
        >
          ...
          >
            <div className="flex flex-col items-center text-center mb-6">
              ...
              <h3 id={`delete-title-${projectId}`} className="text-2xl font-bold text-zinc-900 dark:text-white mb-2">Delete Project?</h3>
              <p id={`delete-desc-${projectId}`} className="text-zinc-600 dark:text-zinc-400 text-sm">
