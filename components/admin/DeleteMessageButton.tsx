...
export default function DeleteMessageButton({ messageId }: { messageId: string }) {
  ...
  return (
    <>
      ...
      {isRendered && (
        <div 
          className={`fixed inset-0 z-[60] flex items-center justify-center p-4 transition-all duration-200 ${isOpen ? 'bg-black/60 backdrop-blur-sm' : 'bg-black/0 backdrop-blur-none'}`} 
          onClick={() => !isPending && setIsOpen(false)}
          role="dialog"
          aria-modal="true"
          aria-labelledby={`delete-msg-title-${messageId}`}
        >
          <div 
          ...
          >
            <div className="flex flex-col items-center text-center mb-6">
              ...
              <h3 id={`delete-msg-title-${messageId}`} className="text-2xl font-bold text-zinc-900 dark:text-white mb-2">Delete Message?</h3>
